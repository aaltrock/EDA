...
ff_hidden_layers = [40, 40]; % 40 neurons each in two layers
ff_trn_func = "trainbr"; % Training function
max_epoch = 60; % Max no. of epoch
...
incident_src = readtable('matlab_input_df.xlsx');
% incident_src = table2array(incident_src);

% Extract the independent variables
% vars_x = {'caller_id','opened_by','location','category','subcategory','u_symptom','assignment_group','knowledge','opened_at_weekday','opened_at_day'};
vars_x = {'location','category','subcategory','u_symptom','assignment_group','knowledge'};
src_x = incident_src(:, vars_x);

% Extract the dependent variable
% (specifically closed_taken only as the target)
src_y = incident_src(:, {'closed_code'});
...
ff_net_resampled = train(ff_net_resampled, transpose(trn_x_SMOTE), transpose(trn_y_SMOTE));
